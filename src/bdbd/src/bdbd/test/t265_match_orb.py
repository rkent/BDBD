...
try:
    ...
    #        \|/
    desired_fov = 150
    stereo_fov_rad = desired_fov * (pi/180)
    ...
    while True:
        ...
        if valid:
            ...
            frame_mutex.acquire()
            left_gray = frame_data["left"].copy()
            right_gray = frame_data["right"].copy()
            frame_mutex.release()

            # Undistort and crop the center of the frames
            left_rect = cv2.remap(src = left_gray,
                                          map1 = undistort_rectify["left"][0],
                                          map2 = undistort_rectify["left"][1],
                                          interpolation = cv2.INTER_LINEAR)
            right_rect = cv2.remap(src = right_gray,
                                          map1 = undistort_rectify["right"][0],
                                          map2 = undistort_rectify["right"][1],
                                          interpolation = cv2.INTER_LINEAR)

            left_rect_color = cv2.cvtColor(left_rect, cv2.COLOR_GRAY2RGB)
            right_rect_color = cv2.cvtColor(right_rect, cv2.COLOR_GRAY2RGB)


            orb_image = left_rect
            orb_image_color = left_rect_color
            keys_left, descs_left = detector.detectAndCompute(left_rect, None)
            keys_right, descs_right = detector.detectAndCompute(right_rect, None)
            #kp = detector.detect(orb_image, None)
            #print(dir(kp[0]))
            print('found {}, {} keypoints'.format(len(keys_left), len(keys_right)))
            print('found {}, {} descriptors'.format(len(descs_left), len(descs_right)))

            # remove overlaps from left
            kp = keys_left
            kp_mask = len(kp) * [True]
            ...
            keys_left_deduped = kpf
            descs_left_deduped = [descs_left[i] for i in range(len(kp)) if kp_mask[i]]
            print('len kpf {} len desc_let_deduped {}'.format(len(kpf), len(descs_left_deduped)))

            # match features
            matcher = cv2.DescriptorMatcher_create(cv2.DESCRIPTOR_MATCHER_BRUTEFORCE_HAMMING)
            print(descs_left)
            matches = matcher.match(np.matrix(descs_left_deduped), descs_right)
            print(dir(matches[0]))
            ...
            # draw top matches
            imMatches = cv2.drawMatches(left_rect_color, keys_left_deduped, right_rect_color, keys_right, matches, None)
            cv2.imshow(WINDOW_TITLE, imMatches)
            '''
            for i in range(len(kp)):
                print(' ')
                kp0=kp[i]
            ...
            #kp, des = detector.compute(orb_image, kp)
            '''
            #orb_image2 = cv2.drawKeypoints(orb_image_color, kpf, None, color=(0,255,0), flags=0)

            #cv2.imshow(WINDOW_TITLE, np.hstack((orb_image_color, orb_image2)))
